...
function AllCustomer(){
    ...
    useEffect(()=>{
        axios.get("http://localhost:5172/Customer/get")
        .then((response)=> {
    ...
    function deleteConfirmationPopUpHandler(){
      axios.delete(`http://localhost:5172/customer/remove/${CustomerToDelete}`)
      .then((response)=>{
...
<br/>
    <div className="d-grid gap-2">
        <Button variant="outline-primary" size="lg" type="button"
...
<tr key={cr.id}><td>{cr.code}</td>
<td>{cr.fullName}</td>
<td>{cr.phoneNumber}</td>
<td>{cr.address}</td>
<td>
<Button variant="outline-warning" type="button" onClick={()=>{navigate(`/edit-customer/${cr.id}`)}}>Edit</Button>{' '}
              <Button variant="outline-danger" type="button" onClick={()=>{showDeleteConfirmationHandler(cr.id)}}>Delete</Button>
</td>
</tr>
        ))}
          </tbody>
