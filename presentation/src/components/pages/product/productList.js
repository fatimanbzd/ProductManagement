...
function AllProduct(){
    ...
    function deleteConfirmationPopUpHandler(){
      axios.delete(`http://localhost:5172/product/remove/${productToDelete}`)
      .then((response)=>{
      ...
      {products.map((pr) => (
        <Col key={pr.id}>
          <Card>
            ...
            <Card.Body>
              <Card.Title>{pr.name} - {pr.code}</Card.Title>
             ...
             {pr.description}
              ...
              <Button variant="outline-warning" type="button" onClick={()=>{navigate(`/edit-product/${pr.id}`)}}>Edit</Button>{' '}
              <Button variant="outline-danger" type="button" onClick={()=>{showDeleteConfirmationHandler(pr.id)}}>Delete</Button>
            </Card.Body>            
          </Card>
        </Col>
      ))}
